...
function fetchOpportunities() {
    fetch('/api/opportunities').then(r => r.json()).then(res => {
        const grid = document.getElementById('opportunitiesGrid');
        if (!grid) return;
        grid.innerHTML = '';
        if (res.data && res.data.length > 0) {
            res.data.forEach(op => renderOpportunityCard(op));
        } else {
            grid.innerHTML = '<p style="color: var(--qf-text-light); padding: 20px;">No opportunities found. Click "Add New Opportunity" to create one.</p>';
        }
    });
}
...
function editOpportunity(id) {
    fetch(`/api/opportunities/${id}`).then(r => r.json()).then(res => {
        if (res.error) return;
        const op = res.data;
...
function renderOpportunityCard(op) {
    ...
    const skillsArr = op.skills.split(',').map(s => s.trim()).filter(Boolean);
    ...
    const applicantsCount = op.max_applicants ? `${op.max_applicants} applicants` : '0 applicants';
    const footerHtml = `
        <div class="opportunity-footer" style="flex-wrap: wrap; gap: 12px;">
            <span class="applicants-count">${escapeHtml(applicantsCount)}</span>
            <div style="display:flex; gap:8px;">
                <button class="view-course-btn" style="width: auto; padding: 8px 16px; background:var(--qf-red); color:white;" onclick="deleteOpportunity(${op.id})">Delete</button>
                <button class="view-course-btn" style="width: auto; padding: 8px 16px; background:#4a90d9; color:white;" onclick="editOpportunity(${op.id})">Edit</button>
...
document.getElementById('opportunityForm').addEventListener('submit', function(e) {
    ...
    if (editingOpportunityId) {
        url = `/api/opportunities/${editingOpportunityId}`;
        method = 'PUT';
    }
    ...
    }).catch(err => showToast('Error saving opportunity'));
});
